...
\subsection{Erstmal die Basics}
Weil ich alles vergesse haha.

\begin{definition}
    Sei $\mathcal{M}\subset \mathcal{B}(H)$ eine von Neumann Algebra.
    Das Predual von $\mathcal{M}$ (geschrieben $\mathcal{M}_*$)
    ist der Raum aller $\sigma$-schwachen stetigen
    linearen Funktionale auf $\mathcal{M}$.
\end{definition}


\begin{definition}
    Seien $(v_n)_{n\in\mathbb{N}}$ und $(w_n)_{n\in\mathbb{N}}$
    zwei Folgen in $\mathcal{H}$, so dass gilt:
    \begin{equation}
        \sum_{n}\lVert v_n \rVert^2 < \infty,\quad
        \sum_{n}\lVert w_n \rVert^2 < \infty.
    \end{equation}
    Die $\sigma$-schwache Topologie auf $\mathcal{B(H)}$ ist definiert als
    die lokal konvexe Topologie erzeugt durch folgende
    Seminormen:
    \begin{equation}
        p_{(v_n)_n, (w_n)_n}(A) := \lvert \sum_{n} \langle v_n, A w_n\rangle \rvert,
        \quad A\in\mathcal{B(H)}.
    \end{equation}
\end{definition}

\begin{definition}
    Sei $\mathcal{A}$ eine C*-Algebra.
    Der positive Kegel von $\mathcal{A}$ ist definiert als die Mengel aller
    positiven Elemente aus $\mathcal{A}$. Wir schreiben $\mathcal{A}_+$.
\end{definition}
